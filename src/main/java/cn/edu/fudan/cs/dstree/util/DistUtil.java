...
public class DistUtil {
    ...
    public static double[] minDistTsBinary(String fileName, double[] queryTs) throws IOException {
        //open index file;
        double[][] tss = TimeSeriesFileUtil.readSeriesFromBinaryFileAtOnce(fileName, queryTs.length);
        return minDistTs(tss, queryTs);
    }
...
//        System.out.println("resultLineNo = " + resultLineNo);
    ...
    public static double[] minDistTs(double[][] tsArray, double[] queryTs) {
        double[] ret = new double[tsArray[0].length];
        double ShortestDist = Double.POSITIVE_INFINITY;
        double tempDist;
        for (int i = 0; i < tsArray.length; i++) {
            tempDist = DistUtil.euclideanDist(queryTs, tsArray[i]);
            if (tempDist < ShortestDist) {
                ShortestDist = tempDist;
                System.arraycopy(tsArray[i],0,ret,0,tsArray[i].length);
            }
        }
        return ret;
    }
